...
| ID | 门禁 | 通过标准 | 状态 |
|---|---|---|---|
| P12-001 | Runtime 原生准入 | 本机 V53 APFS 复制版已通过 14 Provider/219 signed release files/55 资料包/1328 evidence/219 closure；Mac mini native-full 仍须重跑完整 1584/0 | `IN_PROGRESS` |
...
| 2026-08-17 | P10 共用日历核心回归补强：新增真太阳时合同测试，锁定 `local_apparent_solar-v1`、经度/均时差修正、`effective_datetime` 与修正后四柱必须来自同一日历事实；合成福建坐标定向回归 `4 passed`，Ruff 通过。该项证明共用时间层不退回民用时，不代表各术数正式断法或用户验收完成。 | 证据就绪，待用户验收 |
