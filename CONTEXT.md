...
本文只定义全仓库共同语言。产品范围与进度看 `docs/CHECKLIST.md`，视觉与设计权威看 `docs/product-authority.md`（现行视觉基线为玄序，含三态表）与 `DESIGN.md`（非视觉条款），算法接入看 `docs/MINGLI_V51_WEB_INTEGRATION.md`。同一个概念不得在页面、API、数据库和后台各叫一套名字。
...
## 视觉与设计权威（2026-08-27 收敛）

**玄序（Xuan Order）**
2026-08-27 用户最终选定的全站唯一视觉与组件基线：道家 UI 设计系统，克制灰阶、功能性朱砂、发丝分隔、连续阅读面；默认亮色，暗色为可选阅读主题，不建立第二套品牌世界。权威指针、交接链与三态表见 `docs/product-authority.md`。玄序不是候选之一，不存在与它并列的其他视觉方向。

**方向 C（Direction C）**
2026-08-14 曾接受的「现代 SaaS 锐感」视觉方向（浅灰底、单蓝点缀），2026-08-27 起 superseded，替代者为玄序。只保留历史引用，不得作为施工、验收或测试钉值依据。

**纸墨方向（Paper-Ink）**
2026-08-21 起在本地未合入工作树中出现的宣纸/墨色视觉方向（含 `ui/tokens.css` 纸墨档案钉值与相关契约断言口径），从未进入 `origin/main`，2026-08-27 起 superseded，替代者为玄序。

**问真八字（reference-only）**
用户提供的两张问真八字截图，仅作八字展示内容层级与渐进披露参考（四柱、主星/藏干/副星/纳音/神煞、时间层、基本排盘/专业细盘/解读笔记的披露节奏）。不复制其品牌、Logo、图标、黑金配色、字体或像素级构图；八字字段只来自自有 Runtime/ViewModel。

**METIS（reference-only）**
紫微盘面结构、时间层、十二宫聚焦、盘面—解读联动与信息密度/响应式的参考站。不引入其算法、真太阳时粗算、判格/四化推断、Prompt、身份、账户或支付实现。

**五行域色（Wuxing Domain Colors）**
玄序扩展层颜色，只编码盘面事实域（五行属性标注）；禁止进入通用按钮、链接、焦点、通用状态或吉凶判断；必须同时有文字/形状等非颜色编码；亮/暗主题分别满足对比度。精确值唯一来源是已接受的五行域色附录（见 `docs/product-authority.md` §3.3）。

**ALGO-GAP**
前端合同标记：自有 Runtime/ViewModel 尚未提供某字段或能力时的显式缺口标识。缺口走诚实空态/待接入态（`unavailable/empty`，无升级 CTA 误用），浏览器不得推算、补造或由参考站截图反推事实。

**设计验证稿（Design Verification Artifact）**
独立设计目录、HTML 验证页、截图与程序化断言构成的设计交接证据。设计验证稿 ≠ 生产页面完成；其通过不代表对应生产代码已施工、已验收或可发布。

**三态（active / superseded / reference-only）**
产品与设计权威的固定生命周期标记：active 为现行权威；superseded 只保留历史引用并必须写明替代者；reference-only 只在限定范围内提供参考、不产生合同。三态表唯一维护在 `docs/product-authority.md` §4。
