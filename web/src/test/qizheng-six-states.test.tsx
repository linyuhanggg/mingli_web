...
describe("qizheng workbench six states", () => {
  it("keeps fake workbench promises marked 待接入 and clickable", () => {
    render(
      <WorkbenchShell
        onBack={() => undefined}
        product={getProductDefinition("qizheng")}
      />,
    );

    expect(screen.getByRole("heading", { name: "七政工作台" })).toBeVisible();
  ...
  it("reaches honest clickable workbench states from the production surfaceState", () => {
    const product = getProductDefinition("qizheng");
...
describe("qizheng result six states", () => {
  ...
  it("maps a 404 result fetch to empty", async () => {
    const fetchMock = vi.fn<typeof fetch>(async (url) => {
      ...
      return jsonResponse(readingSummary("qizheng", "qizheng"));
    });
  ...
  it("maps GET /result 200 empty body to unavailable", async () => {
    const fetchMock = vi.fn<typeof fetch>(async (url) => {
      if (String(url).endsWith("/result")) {
        return jsonResponse(emptyResult("qizheng", "七政"));
      }
      return jsonResponse(readingSummary("qizheng", "qizheng"));
    });
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("maps GET /result 200 source_status=unavailable to unavailable", async () => {
    const fetchMock = vi.fn<typeof fetch>(async (url) => {
      if (String(url).endsWith("/result")) {
        return jsonResponse(emptyResult("qizheng", "七政", "unavailable"));
      }
      return jsonResponse(readingSummary("qizheng", "qizheng"));
    });
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("keeps an accepted_copy result page instead of empty or unavailable", async () => {
    const fetchMock = vi.fn<typeof fetch>(async (url) => {
      if (String(url).endsWith("/result")) {
        return jsonResponse(acceptedResult("qizheng", "七政"));
      }
      return jsonResponse(readingSummary("qizheng", "qizheng"));
    });
    vi.stubGlobal("fetch", fetchMock);
