...
Three transmissions are calculated by the local `classical_nine-method_algorithm` under rule profile `daliuren-daquan-wyg-classical-nine-method-v2`: 贼克、比用、涉害、遥克、昴星、别责、八专、伏吟 and 反吟 each have an auditable branch. 涉害 records every traversed ground branch, lodged stem, depth, 孟仲季 tie-break, and 复等 fallback. Each transmission also carries `hidden_stem` under `sexagenary-day-xun-v1`: the occupied stem in the cast day's xun, or null only when that transmission branch is one of the two Xunkong branches. This is a deterministic plate fact, not a verdict. The adapter vendors the Apache-2.0 `look-fate/liuren-ts-lib` 60x12 table at fixed commit `8e9a7b5` only as a cross-check witness. A full audit finds four method-label disagreements and sixteen transmission-result disagreements; the table never overrides the classical calculation. Normal casts embed only the audit counts; use `python3 scripts/liuren_fact_adapter.py audit-table` for the full 720-record disagreement list and `python3 scripts/audit_liuren_transmission_hidden_stems.py` for all 60×12 xun mappings.
...
| `san-shi/liuren` | calendar_normalization, 天地盘, `four_lessons`, `three_transmissions`（含旬内 `hidden_stem`）, `heavenly_generals`, `month_general`, `day_hour`, 旬空, rule profile, conflicts | one concrete question, date/time, place/timezone, Zi-hour and guiren conventions | first read `liuren-casting-gate.md`, run `liuren_fact_adapter.py`, then load `daliuren-daquan`, `liuren-zhiyin`, `liuren-miben` |
