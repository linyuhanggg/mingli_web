...
def compile_time_check_prepare(
    *,
    ...
    dimension_ids: tuple[str, ...],
    known_event_facts: tuple[Mapping[str, object], ...] = (),
) -> Prepare:
    ...
    the tool supplies the known clock range, optional labels, and optional
    dated event facts. Runtime owns candidate calculation and bounded evidence
    ranking; it does not produce a classical life-event verdict.
    """
    ...
    if known_event_facts:
        facts["known_event_facts"] = [dict(event) for event in known_event_facts]
    return Prepare(
        query=query,
        intent=_intent(
            subject_ref=profile.subject_ref,
    ...
    *,
    ...
    if target_relative is not None and target_relative not in _LIUREN_TARGET_RELATIVES:
        raise RequestCompilationError(
            ...
            f"{sorted(_LIUREN_TARGET_RELATIVES)!r}"
        )
