...
| Task 2 数据正确性 | ✅ Step 1–5 全部完成（幂等 @ d72c302，Profile 语义 @ 064f45a，三条核对 @ 49bee46，免费范围交付 @ 6c85093），待审查 | 幂等并发 bug 已修复：RED 复现 → 迁移 `0009_idempotency_owner_uniqueness` 两个 partial unique index → GREEN。Profile 编辑=同 Root 追加不可变版本（`POST /profiles/{id}/versions`），出生合同补齐历法/闰月/时辰准确度/坐标来源精度，真太阳时强制显式坐标。Step 4：核对改为三条 fact_ref 级独立结果（迁移 `0010_verification_results`，服务端按公开事实面板校验 ref，未知/重复→400，未 accepted→409；Web 逐条核对，不足三条不收集；答案不回灌 Prompt）。Step 5（经用户裁决推翻事业化收窄）：免费八字概览按蓝图实际交付 overview+state——`PREVIEW_V1` required=overview、新增 `SCOPED_PREVIEW_V1`（career）按 capability 选用、`profile_preview` 编译层钉死 {overview,state}、openapi 预览枚举 [overview,state]、全站恢复「八字概览」与 overview+state payload。backend 408 passed + PG 418 passed + web 254 passed + ruff/tsc/eslint/build 全绿。mypy 有 3 个 HEAD 遗留 error（admin role×2、service horizon×1），与本步无关 |
...
**Step 5: 冻结免费解读范围** ✅（@ 6c85093，推翻 6d9943f 的事业化收窄；用户裁决：网站基于命理技能核心，免费概览必须按蓝图实际交付本命整体概览，而不是把承诺改小。Blueprint §10.3：免费概览 `dimension_ids`=overview+state；§4.1：八字五维词汇 overview/state/career/relationship/timing 留给付费深读。交付侧对齐：`PREVIEW_V1` required=overview，新增 `SCOPED_PREVIEW_V1`（required=career）供今日/近七日/六爻按 capability 选用；`compile_bazi_prepare` 对 `profile_preview` 钉死 {overview,state}（career-only→400），`bazi_deep` 保留五维；`DEFAULT_QUERIES["profile_preview"]`「请预览我的本命八字概览。」；服务默认维度 ("overview","state")；openapi 预览枚举改 [overview,state]；Web 恢复「八字概览」名词与 overview+state payload（`bazi-flow.tsx`/`profile-archive.tsx`/`dashboard-hub.tsx`/`app/app/page.tsx`/`app/app/bazi/page.tsx`/`product-capabilities.ts`），`reading-display.ts` 补 state/relationship 标签。TDD：backend 27 处 RED + web 5 处 RED 先验失败，门禁：backend 408 passed + PG 418 passed + ruff 绿（mypy 仅 3 个 HEAD 既有遗留）、web 254 passed + lint/tsc/build 全绿。遗留：今日/近七日与六爻仍为 career 范围（本次未动），蓝图称六爻免费根请求为基础 state 范围，待用户决策）
