...
def verify_release(
    ...
    *,
    ...
    required_keys = {"schema_version", "files", "patterns"}
    allowed_keys = required_keys | {"excluded_files"}
    if (
        not required_keys <= set(closure_payload) <= allowed_keys
        or closure_payload.get("schema_version") != RUNTIME_CLOSURE_SCHEMA
    ):
        raise IdentityError("Runtime closure contract is invalid")
    raw_files = closure_payload.get("files")
    raw_patterns = closure_payload.get("patterns")
    raw_excluded_files = closure_payload.get("excluded_files", [])
    if (
        ...
        raise IdentityError("Runtime closure contract is invalid")
    try:
        files = tuple(_safe_relative(item, "Runtime closure") for item in raw_files)
        patterns = tuple(_safe_release_pattern(item) for item in raw_patterns)
        excluded_files = tuple(
            _safe_relative(item, "Runtime closure exclusion")
            for item in raw_excluded_files
        )
    except (TypeError, IdentityError) as exc:
        raise IdentityError("Runtime closure contract is invalid") from exc
    if (
        not files
        or len(files) != len(set(files))
        or len(patterns) != len(set(patterns))
        or len(excluded_files) != len(set(excluded_files))
        or set(files) & set(excluded_files)
    ):
        raise IdentityError("Runtime closure contract is invalid")
    ...
    for pattern in patterns:
        ...
        if not matches and not excluded_matches:
            raise IdentityError("Runtime closure pattern matched no declared files")
        selected.update(matches)
        matched_exclusions.update(excluded_matches)
    unmatched_exclusions = set(excluded_files) - matched_exclusions
    if unmatched_exclusions:
        raise IdentityError("Runtime closure exclusion matched no pattern")
    selected.difference_update(excluded_files)
    # The signed manifest and closure must describe exactly the same files.
    # Explicit exclusions describe tracked source-only modules and therefore
    # must never appear in the installed release. Tests remain outside it.
