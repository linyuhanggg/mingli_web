...
export function ProductInputForm({
  ...
  showFrozenUnknownTimeControl?: boolean;
}) {
  ...
  // The integrated compact Bazi page omits dead controls; standalone form previews expose the frozen option.
  const shouldShowFrozenUnknownTimeControl =
    showFrozenUnknownTimeControl
    ?? (product.id !== "bazi" || onProfileVersionChange === undefined);
  ...
  return (
    ...
    >
      ...
      {isCompactBaziInput && shouldShowFrozenUnknownTimeControl ? (
        <p className={styles.productNote}>确认后生成盘面</p>
      ) : null}
      ...
      {(product.group === "natal" || usesCrossProfile) && !selectedProfileVersionId && !profileLookupPending ? (
        <fieldset className={styles.fieldGroup}>
          ...
          {shouldShowFrozenUnknownTimeControl ? (
            <label className={styles.checkRow}>
              <input
                aria-describedby={`${product.id}-unknown-time-reason`}
                disabled
                type="checkbox"
                {...register("unknownTime")}
              />
              <span>
                <strong>不知道出生时辰</strong>
                <small id={`${product.id}-unknown-time-reason`}>
                  请填写明确的出生时间。
                </small>
              </span>
            </label>
