...
describe("public visual contract", () => {
  ...
  it("uses the shared neutral canvas without legacy product colors", () => {
    expect(ruleFor(homeCss, ".main")).toMatch(/background:\s*var\(--color-canvas\)/);
    expect(ruleFor(homeCss, ".card")).toMatch(/border:\s*1px solid var\(--color-border\)/);
    expect(ruleFor(homeCss, ".card")).toMatch(/background:\s*var\(--color-surface\)/);
    expect(ruleFor(homeCss, ".observation p")).toMatch(
      /color:\s*(?:var\(--color-(?:text-inverse|on-action)\)|color-mix\([^;{}]*var\(--color-(?:text-inverse|on-action)\))/,
    );
    expect(homeCss).not.toMatch(/gold|terracotta|moss|amber/);
