...
@pytest.mark.parametrize(
    ("mode", "region", "descriptor", "taxonomy"),
    [
...
def test_non_face_modes_emit_their_declared_runtime_contract(
    mode: str,
    region: str,
    descriptor: str,
    taxonomy: str,
    ...
    spec = runtime_input.physiognomy_spec
    assert spec["observation_scope"] == mode
    assert spec["requested_targets"][0]["taxonomy"] == taxonomy  # type: ignore[index]
    assert spec["requested_targets"][0]["region"] == region  # type: ignore[index]


@pytest.mark.parametrize(
    ("mode", "region", "descriptor"),
    [
        ("palm", "left_palm", "line_continuous"),
        ("palm", "life_line", "ridge_visible"),
    ],
)
def test_palm_descriptor_contract_rejects_cross_region_values(
    mode: str,
    region: str,
    descriptor: str,
) -> None:
