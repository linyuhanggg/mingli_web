      ...
      {state === "ready" && data ? (
        <section className={styles.panel} aria-labelledby="reading-detail-title">
          ...
          <dl className={styles.details}>
            ...
            <div>
              <dt>产品</dt>
              <dd>{data.product_id ?? data.capability_id}</dd>
            </div>
            <div>
              <dt>版本</dt>
              <dd>{data.version}</dd>
            ...
            {data.document_view_model_schema ? (
              <div>
                <dt>文档 ViewModel</dt>
                <dd>{data.document_view_model_schema}</dd>
              </div>
            ) : null}
            {data.physiognomy_source_summary ? (
              <div>
                <dt>见相来源</dt>
                <dd>
                  {data.physiognomy_source_summary.source_count} 层，保留{" "}
                  {data.physiognomy_source_summary.disagreement_count} 项分歧，
                  {data.physiognomy_source_summary.disagreements_retained ? "未强行合并" : "无分歧记录"}
                </dd>
              </div>
            ) : null}
            {data.time_check_summary ? (
              <div>
                <dt>寻时定盘摘要</dt>
                <dd>
                  {data.time_check_summary.candidate_count} 个候选，
                  {data.time_check_summary.known_event_count} 条结构化事件；
                  {data.time_check_summary.ranked_candidate_count} 条排序，
                  {data.time_check_summary.event_match_count} 条事件匹配，
                  {data.time_check_summary.event_matching_status === "structured_evidence"
                    ? "已形成结构化证据"
                    : "未计算事件匹配"}
                </dd>
              </div>
