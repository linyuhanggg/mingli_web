...
> **2026-08-31 dated addendum（MING-75 F-08 冻结裁决）：** 上句“全局登录弹层是主要入口”已废止。认证继续使用页面式 `/auth/*`，公共 Header 的登录入口与账户区游客引导是现行入口；本阶段不新增全局登录弹层，也不改变既有认证、账户或支付语义。
...
> **2026-08-31 dated addendum（MING-75 F-04 冻结裁决）：** “固定五项底栏”只表示导航项固定为主页、术数、工具、每日、我的五项，不表示 CSS `position: fixed`。移动底栏按 HANDOFF-v3 §2.2 置于 `main` 后的 static 文档流，禁止 fixed/sticky，并以滚动全过程与主内容/主行动构造性零相交为验收；不得靠页面末尾 padding 掩盖遮挡。
