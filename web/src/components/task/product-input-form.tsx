...
function schemaFor(product: ProductDefinition) {
  ...
  return taskSchemaBase.superRefine((values, context) => {
    ...
    if (product.id === "jianxiang") {
      if (!values.subject.trim()) context.addIssue({ code: "custom", path: ["subject"], message: "请填写受测对象" });
      if (values.observationMode !== "face") context.addIssue({ code: "custom", path: ["observationMode"], message: "当前 Runtime 只接入面相观察" });
...
export function ProductInputForm({
  ...
  const handleInvalid = (invalidErrors: FieldErrors<TaskFormValues>) => {
    const firstError = Object.keys(invalidErrors)
      .map((fieldName) => taskErrorField(product.id, fieldName, invalidErrors))
      .find((field): field is { id: string; label: string } => Boolean(field));
    ...
    <form aria-label={`${product.name}任务输入`} className={styles.formPanel} noValidate onSubmit={handleSubmit(onConfirm, handleInvalid)}>
      ...
      {product.id === "jianxiang" ? (
        <fieldset className={styles.fieldGroup}>
          ...
          <Field htmlFor="jianxiang-mode" label="观照模式" help="各分支先分别呈现观察，再显示共同印证与分歧。">
            <select id="jianxiang-mode" {...register("observationMode")}>
              <option value="face">面相（当前可用）</option>
              <option disabled value="palm">手相（待对应 Runtime 接入）</option>
              <option disabled value="posture">体态（待对应 Runtime 接入）</option>
              <option disabled value="combined">综合观照（待对应 Runtime 接入）</option>
            </select>
          </Field>
          <fieldset className={styles.fieldGroup}>
            ...
            <div className={styles.twoColumns}>
                ...
                <select
                  ...
                  {...register("observationRegion", {
                    onChange: (event) => {
                      ...
                      setValue("observationDescriptor", FACE_DESCRIPTOR_OPTIONS[nextRegion]?.[0] ?? "region_visible", { shouldDirty: true, shouldValidate: true });
                    },
                  })}
                >
                  {Object.keys(FACE_DESCRIPTOR_OPTIONS).map((region) => <option key={region} value={region}>{region}</option>)}
                </select>
              </Field>
              <Field htmlFor="jianxiang-observation-descriptor" label="观察描述" error={errors.observationDescriptor?.message}>
                <select id="jianxiang-observation-descriptor" {...register("observationDescriptor")}>
                  {(FACE_DESCRIPTOR_OPTIONS[observationRegion] ?? ["region_visible"]).map((descriptor) => <option key={descriptor} value={descriptor}>{descriptor}</option>)}
                </select>
              </Field>
            </div>
            <div className={styles.twoColumns}>
          ...
          <div className={styles.capturePanel}>
            ...
            <div className={styles.captureActions}>
              ...
              {photoName ? <button className={styles.secondaryButton} onClick={clearPhoto} type="button">删除本地照片</button> : null}
            </div>
            <input
              ref={fileInputRef}
