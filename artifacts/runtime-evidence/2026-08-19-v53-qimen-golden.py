#!/usr/bin/env python3
"""Read-only: P10-007 奇门黄金样例 — signed V53 vs core."""
...
def main() -> int:
    ...
    impl = [
        "scripts/reading_engine/qimen.py",
        "resources/runtime/providers/qimen.json",
    ...
    fx = [
        "qimen-v51.yaml",
        "qimen-go-v51.yaml",
        "qimen-pattern-contract-v51.yaml",
    ]
    fx_signed = {n: (SIGNED / "references/fixtures" / n).exists() for n in fx}
    print(f"inspector={inspector}")
    print(f"files={nfiles}")
    print(f"qimen_cu_signed={[x for x in signed_cu if x.startswith('qimen.')]}")
    print(f"qimen_cu_core={[x for x in core_cu if x.startswith('qimen.')]}")
    print(f"impl_files_identical={same}")
    print(f"qimen_packs={packs}")
