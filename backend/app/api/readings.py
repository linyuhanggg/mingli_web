...
@router.post(
    "/life-kline-series",
    operation_id="startLifeKlineSeriesReading",
    response_model=ReadingStartResponse,
    status_code=status.HTTP_201_CREATED,
)
async def start_life_kline_series_reading(
    request: Request,
    response: Response,
    payload: LifeKlineSeriesStartRequest,
    session: AsyncSession = Depends(database_session),
    owner: Owner = Depends(require_owner_csrf),
    idempotency_key: str | None = Header(
        default=None,
    ...
    _check_dogfood_daily_limits(owner, request, paid=False)
    try:
        result = await _service(request, session).start_life_kline_series(
            owner,
            profile_version_id=payload.profile_version_id,
            query=payload.query,
            dimension_ids=payload.dimension_ids,
