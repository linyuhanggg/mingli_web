...
function mapByEarth(
  ...
  valueKey: "heaven_branch" | "general",
): ReadonlyMap<string, string> {
  const mapped = new Map<string, string>();
  if (!items) return mapped;
  for (const item of items) {
    const earth = readString(item, "earth_branch");
...
export function DaliurenHeavenEarthPlate({
  ...
  const heavens = mapByEarth(heavenPlate, earthSet, "heaven_branch");
  ...
  const noble =
    readString(noblePerson, "branch") ?? readString(noblePerson, "earth_branch");
