...
def project_time_check_view_model(
    ...
    """Project Runtime-owned twelve-hour candidate facts without ranking."""
    ...
    if (
        ...
        or ranking_fact[1] != "not_ranked"
        or matching_fact[1] != "not_calculated"
    ):
        return None
    try:
        candidates = tuple(
    ...
    return TimeCheckViewV1(
        ...
        limitations=(
            "当前只输出十二个时辰候选的八字与时间口径事实，不做事件匹配或候选淘汰。",
...
def _meihua_core_facts(facts: object) -> MeihuaCoreFacts | None:
    ...
    if parsed_body_relations is None and parsed_seasonal_strength is None and parsed_status is None:
        return None
...
def project_selection_view_model(
    ...
    eligible_date_times = _taiyi_string_tuple(
...
def project_canwen_view_model(
    ...
    for raw_dimension in raw_dimensions:
        ...
        if len(signals) == len(selected_art_ids):
